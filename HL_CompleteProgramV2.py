        ...
        while guesses_left >= 1 and secret_num != guess:
            ...
            if guesses_left >= 1:  # Compares users guess to the secret number and gives appropriate feedback
                if guess < secret_num:  # If guess is lower than secret number tell user to go higher
                    if guesses_left > 1:
                         hl_statement("^^ Higher | {} guesses left ^^".format(guesses_left), "^")  # Too Low Feedback
                    else:
                         hl_statement("^^ Higher | {} guess left ^^".format(guesses_left), "^")   # Grammatically correct too low feedback for only one guess

                elif guess > secret_num:  # If guess is higher than secret number tell user to go lower
                    if guesses_left > 1:
                        hl_statement("vv Lower | {} guesses left vv".format(guesses_left), "v")  # Too High feedback
                    else:
                        hl_statement("vv Lower | {} guess left vv".format(guesses_left), "v")  # Grammatically correct  too high feedback for only one guess
                    ...
                    if guesses_left == maximum_guesses - 1:
                        ...
                        hl_statement("!! Congratulations, you got it right in {} guesses !!".format(maximum_guesses - guesses_left), "!")  # Normal Win statement
